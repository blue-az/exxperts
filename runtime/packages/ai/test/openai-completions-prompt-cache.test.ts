...
describe("openai-completions prompt caching", () => {
	...
	it("uses split OpenAI prompt cache retention support even when legacy long retention is disabled", async () => {
		const model = createModel({
			baseUrl: "https://proxy.example.com/v1",
			compat: {
		...
		const { payload } = await captureRequest({ cacheRetention: "long", sessionId: "session-split-openai" }, model);

		expect(payload?.prompt_cache_key).toBe("session-split-openai");
		expect(payload?.prompt_cache_retention).toBe("24h");
	});

	it("does not use Anthropic TTL support as OpenAI prompt cache retention support", async () => {
		const model = createModel({
			baseUrl: "https://proxy.example.com/v1",
			compat: {
				supportsLongCacheRetention: false,
		...
		const { payload } = await captureRequest({ cacheRetention: "long", sessionId: "session-anthropic-only" }, model);
