...
function createNpmPrefixInstall(
	template = "pi-prefix-",
	layout: "posix" | "native" = "posix",
): { prefix: string; packageDir: string } {
	const prefix = mkdtempSync(join(tmpdir(), template));
	// Windows npm prefixes use `<prefix>\node_modules` (no `lib`); the "native"
	// layout mirrors what `npm root -g --prefix <prefix>` reports on the platform.
	const root =
		layout === "native" && process.platform === "win32"
			? join(prefix, "node_modules")
			: join(prefix, "lib", "node_modules");
	const scopeDir = join(root, "@exxeta");
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@exxeta+exxperts-runtime@0.67.68\\node_modules\\@exxeta\\exxperts-runtime\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@exxeta/exxperts-runtime")).toBe(
			"Run: pnpm install -g @exxeta/exxperts-runtime",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@exxeta/exxperts-runtime")).toBeUndefined();
		expect(getUpdateInstruction("@exxeta/exxperts-runtime")).toBe(
			"Update @exxeta/exxperts-runtime using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@exxeta/exxperts-runtime");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "@exxeta/exxperts-runtime"],
			display: `npm --prefix ${prefix} install -g @exxeta/exxperts-runtime`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		const { prefix } = createNpmPrefixInstall("pi-prefix-", "native");

		const command = getSelfUpdateCommand("@exxeta/exxperts-runtime", ["npm", "--prefix", prefix]);
	...
	test("self-update treats empty npmCommand as unset", () => {
		...
		expect(command?.args).toEqual(["--prefix", prefix, "install", "-g", "@exxeta/exxperts-runtime"]);
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(`npm --prefix "${prefix}" install -g @exxeta/exxperts-runtime`);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@exxeta\\exxperts-runtime";
		process.env.PI_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@exxeta/exxperts-runtime")).toBe(
			"Run: npm install -g @exxeta/exxperts-runtime",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "@exxeta/exxperts-runtime"],
			display: "bun install -g @exxeta/exxperts-runtime",
		});
	});
	...
	// chmod is a no-op for directories on Windows, so the path stays writable there.
	test.skipIf(process.platform === "win32")("does not self-update when npm install path is not writable", () => {
		const { packageDir } = createNpmPrefixInstall();
		chmodSync(packageDir, 0o500);

		expect(getSelfUpdateCommand("@exxeta/exxperts-runtime")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@exxeta/exxperts-runtime")).toContain(
