...
// The suspend path is Unix-only; stub the platform so these fully mocked
// tests exercise it on Windows too.
function callHandleCtrlZAsLinux(context: HandleCtrlZThis): void {
...
describe("InteractiveMode.handleCtrlZ", () => {
	...
	test("keeps the process alive while suspended and restores the TUI on SIGCONT", () => {
		...
		callHandleCtrlZAsLinux(context);
	...
	test("cleans up the temporary handlers if suspension fails", () => {
		...
		expect(() => callHandleCtrlZAsLinux(context)).toThrow(suspendError);
		expect(ui.stop).toHaveBeenCalledTimes(1);
