...
describe("issue #2791 fs.watch error event crashes process", () => {
	...
	it("process should survive an error event on the theme FSWatcher", () => {
		// Import via a file:// URL so the specifier is valid on Windows too.
...
`,
		...
		let stderr = "";
		let exitCode: number;
		// Resolve tsx through the module system and run it with the current node binary;
		// spawning "npx" directly fails on Windows (npx is a .cmd shim).
