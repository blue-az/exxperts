...
describe("createAgentSession session manager defaults", () => {
	...
	it("derives cwd from an explicit sessionManager when cwd is omitted", async () => {
		...
		expect(session.sessionManager).toBe(sessionManager);
		// The system prompt normalizes the working directory to forward slashes.
		...
		if (process.platform === "win32") {
			// Git Bash reports a POSIX-style $PWD (e.g. /tmp/...), so compare the stable tail.
			const expectedTail = sessionCwd.split(sep).slice(-2).join("/");
			expect(output.trim().split("/").slice(-2).join("/")).toBe(expectedTail);
		} else {
