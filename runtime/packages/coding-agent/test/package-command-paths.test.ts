...
import { APP_NAME, CONFIG_DIR_NAME, ENV_AGENT_DIR, PACKAGE_NAME, VERSION } from "../src/config.js";
...
describe("package commands", () => {
	...
	it("uses global npmCommand and current package name for forced self updates without checking the api", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@exxeta", "exxperts-runtime");
...
`,
		...
		try {
			...
			expect(process.exitCode).toBeUndefined();
			expect(errorSpy).not.toHaveBeenCalled();
			// The upstream release API is disabled in the exxperts fork, so no version
			// check request is made and the self-update falls back to the current package name.
			expect(fetchMock).not.toHaveBeenCalled();
			const recordedArgs = JSON.parse(readFileSync(recordPath, "utf-8")) as string[];
			expect(recordedArgs).toContain(PACKAGE_NAME);
		} finally {
	...
	// The upstream release API (getLatestPiRelease) is disabled in the exxperts fork,
	// so the renamed-package self-update flow driven by the API is unreachable.
...
`,
		...
		try {
			...
			// The upstream release API is disabled in the exxperts fork, so the
			// self-update installs the current package name without an uninstall step.
			const recordedCalls = JSON.parse(readFileSync(recordPath, "utf-8")) as string[][];
