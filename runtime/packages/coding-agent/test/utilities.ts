...
import { getModel, type OAuthCredentials, type OAuthProvider } from "@exxeta/exxperts-ai";
...
export const symlinksSupported = (() => {
	...
	const dir = join(tmpdir(), `pi-coding-agent-symlink-probe-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	try {
