...
async function deleteSessionFile(
	...
	// Try `trash` first (if installed); on Windows there is no `trash` CLI, so
	// send the file to the Recycle Bin via PowerShell instead.
	let trashResult: SpawnSyncReturns<string>;
	if (process.platform === "win32") {
		const escaped = sessionPath.replace(/'/g, "''");
		trashResult = spawnSync(
			"powershell.exe",
			[
				"-NoProfile",
				"-NonInteractive",
		...
		trashResult = spawnSync("trash", trashArgs, { encoding: "utf-8" });
	}
