...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@exxeta/exxperts-runtime` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@exxeta/exxperts-runtime";
	 *
...
export interface ExtensionContext {
	...
	/** Per-agent context policy (compaction thresholds, preserve instructions, injection rules). Undefined if no agent-level policy is configured. */
...
export interface BeforeAgentStartEvent {
	...
	/** Per-agent context policy. Extensions can use alwaysInject/lazyLoad to decide what additional context to include. */
	contextPolicy?: ContextPolicy;
}
...
export interface ExtensionContextActions {
	...
	compact: (options?: CompactOptions) => void;
	getSystemPrompt: () => string;
	getContextPolicy: () => ContextPolicy | undefined;
}
