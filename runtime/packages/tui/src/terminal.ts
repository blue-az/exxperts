...
export class ProcessTerminal implements Terminal {
	...
	private keyboardFallbackTimer?: ReturnType<typeof setTimeout>;
	...
	private setupStdinBuffer(): void {
		...
		this.stdinBuffer.on("data", (sequence) => {
			...
			if (!this._kittyProtocolActive) {
				...
				if (match) {
					this.clearKeyboardFallbackTimer();
	...
	private queryAndEnableKittyProtocol(): void {
		...
		this.keyboardFallbackTimer = setTimeout(() => {
			this.keyboardFallbackTimer = undefined;
	...
	private clearKeyboardFallbackTimer(): void {
		if (!this.keyboardFallbackTimer) return;
		clearTimeout(this.keyboardFallbackTimer);
		this.keyboardFallbackTimer = undefined;
	}
	...
	async drainInput(maxMs = 1000, idleMs = 150): Promise<void> {
		this.clearKeyboardFallbackTimer();
