...
export interface EditorTheme {
	...
	/** Colour for the prompt caret shown when the editor is empty. */
	prompt?: (str: string) => string;
	/** Colour for the placeholder hint shown when the editor is empty. */
...
export interface EditorOptions {
	...
	/** Hint text shown (dimmed) while the editor is empty. */
	placeholder?: string;
}
...
export class Editor implements Component, Focusable {
	...
	private placeholderColor: (str: string) => string;
	...
	constructor(tui: TUI, theme: EditorTheme, options: EditorOptions = {}) {
		...
		this.promptColor = theme.prompt ?? theme.borderColor;
		this.placeholderColor = theme.placeholder ?? theme.borderColor;
	...
	render(width: number): string[] {
		...
		// Empty-state: show a prompt caret + dimmed placeholder hint. This only
		// applies when there is no user text, so it never affects cursor math.
		const showPlaceholder = this.placeholder && this.getText() === "" && !this.autocompleteState;
		if (showPlaceholder) {
			const caret = this.promptColor("› ");
			const marker = emitCursorMarker ? CURSOR_MARKER : "";
			const hintWidth = Math.max(1, contentWidth - visibleWidth("› "));
			const hint = this.placeholderColor(truncateToWidth(this.placeholder as string, hintWidth));
			const lineText = `${caret}${marker}${hint}`;
			const padding = " ".repeat(Math.max(0, contentWidth - visibleWidth(`› ${this.placeholder}`)));
			result.push(`${leftPadding}${lineText}${padding}${rightPadding}`);
			result.push(horizontal.repeat(width));
			return result;
		}
