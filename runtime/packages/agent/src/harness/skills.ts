...
// Env paths may use either separator (Windows local env vs POSIX), so these
// helpers treat "/" and "\" alike instead of using node:path, which would
// assume the host platform for possibly-remote execution environments.
...
function relativeEnvPath(root: string, path: string): string {
	...
	if (normalizedPath.startsWith(normalizedRoot) && /[\\/]/.test(normalizedPath.charAt(normalizedRoot.length))) {
