...
import { createTempDir, symlinksSupported } from "./session-test-utils.js";
...
describe("NodeExecutionEnv", () => {
	...
	it.skipIf(!symlinksSupported)("returns fileInfo for files, directories, and symlinks without following symlinks", async () => {
		const root = createTempDir();
		const env = new NodeExecutionEnv({ cwd: root });
	...
	it.skipIf(!symlinksSupported)("lists symlinks as symlinks", async () => {
		const root = createTempDir();
		const env = new NodeExecutionEnv({ cwd: root });
	...
	it("executes commands in cwd with env overrides", async () => {
		...
		if (process.platform === "win32") {
			// Git Bash reports $PWD in POSIX form and mounts the Windows temp
			// dir as /tmp, so only the directory name is stable to assert on.
			expect(result.stderr).toBe("");
			expect(result.exitCode).toBe(0);
			expect(result.stdout.endsWith(`/${basename(real)}:ok`)).toBe(true);
		} else {
			expect(result).toEqual({ stdout: `${real}:ok`, stderr: "", exitCode: 0 });
		}
	});
